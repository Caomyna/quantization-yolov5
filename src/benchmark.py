"""
Benchmark FP32 vs FP16 ONNX models: latency, throughput, memory.
"""
...
from config import ONNX_FP32_PATH, ONNX_FP16_PATH, DATASET_DIR, BENCHMARK_CONFIG
...
class ONNXInferenceBenchmark:
    ...
    def __init__(self, model_path: Path, model_name: str):
        ...
        logger.info(f"Loaded {model_name}: input={self.input_name} {self.input_shape}")
...
def benchmark_models(
    ...
    fp16_path: Path = ONNX_FP16_PATH,
    dataset_dir: Path = DATASET_DIR,
    config: dict = None,
) -> Dict[str, Any]:
    """Full benchmark pipeline: load images → benchmark both → compare → save."""
    cfg = config or BENCHMARK_CONFIG
    warmup = cfg["warmup_iterations"]
    iterations = cfg["num_iterations"]

    logger.info("=" * 50)
    logger.info("Benchmark FP32 vs FP16")
    logger.info("=" * 50)

    images = load_test_images(dataset_dir)

    fp32_metrics = ONNXInferenceBenchmark(fp32_path, "FP32").benchmark(images, warmup, iterations)

    fp16_metrics = None
    try:
        fp16_metrics = ONNXInferenceBenchmark(fp16_path, "FP16").benchmark(images, warmup, iterations)
    ...
    if fp16_metrics:
        ...
        logger.info("FP16: skipped (requires CUDA)")
    logger.info(f"Size reduction: {comparison['size_reduction_pct']:.1f}%")

    base = Path("benchmark") / "benchmark_results"
    save_results(fp32_metrics, fp16_metrics, comparison, base.with_suffix(".json"))
    save_csv(fp32_metrics, fp16_metrics, comparison, base.with_suffix(".csv"))
